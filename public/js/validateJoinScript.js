/* Filename:for validate form inputs*/
function validateInputs() {
    var username = $("#userName").val();
    var password = $("#password").val();
    
    if ($.trim(username) == '' || $.trim(password) == '') {  alert("Both username and password are required");
        return false;
    }
    //username validation
    ...
    if (password.length < 4) { alert("passowrd too short");
        return false;
    }
    // calling the reserved usernames
    if (reservedUsernames(username) != -1) { alert("Username not allowed");
        return false;
    }
    if (username == password) { alert("username and password cannot be the same");
        return false;
    }
    return true;
}
...
function validateEmergency() {
    var emergencyTitle = $("#emergencyTitle").val();
    var description = $("#description").val();

    if ($.trim(emergencyTitle) == '' || $.trim(description) == '' || $.trim(itemName) == '' || $.trim(quantity) == '' || $.trim(quantity) == '') {
        alert("ALL FIELDS ARE REQUIRED");
        return false;
    }

    if (emergencyTitle.length < 10) { alert(" EMERGENCY TITLE MUST BE AT LEAST 10 CHARACTOR LONG");
        return false;
    }

    if (description.length > 100) { alert(" EMERGENCY DESCRIPTION MUST BE AT MOST 100 CHARACTER LONG");
        return false;
    }
    return true;
}
